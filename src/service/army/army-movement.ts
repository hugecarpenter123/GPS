...
export default class ArmyMovement {
  ...
  public async performAttack(operationDetails: AttackOperationDetails) {
    ...
    await this.openCityOperationDialog(operationDetails.id, operationDetails.targetCitySelector, operationDetails.operationType)
  }

  private async openCityOperationDialog(locationId: string, targetCitySelector: string, operationType: Omit<OperationType, OperationType.WITHDRAW>) {
    ...
    const dropdownItem = Array.from(dropdownList.querySelectorAll<HTMLElement>(`.item.bookmark.option`))
      .find(el => el.textContent?.trim() === locationId);
    if (!dropdownItem) {
  ...
  private async fillInputData(inputData: { value: string, name: string }[], attachHero?: boolean, charm?: CharmDetails) {
    ...
    if (charm) {
      ...
      await waitForElementInterval(`[data-power_id="${charm.dataPowerId}"`, { interval: 500, timeout: 2000 }).then(el => (el as HTMLElement).click());
    }
  }
