...
export default class MunicipalUtility implements Service, MasterQueueService<MunicipalUtilityQueueItem> {
  ...
  private async tryPerformOrScheduleOperation(operationDetails: ScheduleOperationDetails<MunicipalUtilityQueueItem>) {
    const { city, queueItem: { operationType } } = operationDetails;
    ...
    try {
      ...
      // not blocked by counter
        ...
        if (operationPerformDetials.requiredResources && Object.values(operationPerformDetials.requiredResources).some(Boolean)) {
          // TODO: get supplier cities to pass as an argument
          const stackResult = await this.tradeManager.stackResources(operationPerformDetials.requiredResources, city, [] as CityInfo[], 20000);
      ...
      this.generalInfo.showError('MunicipalUtility:', `Something went wrong when performing "${operationDetails.queueItem.operationType}"`);
   ...
   */
  private async getCanPerformDetails(operation: MunicipalEvent): Promise<{ canPerform: boolean, timeToPerform?: number, requiredResources?: ResourcesInfo }> {
  ...
  private async performOperation(operation: MunicipalEvent): Promise<boolean> {
    ...
    if (operation === MunicipalEvent.CityFestival) {
      ...
      await waitWhile(() => !document.querySelector<HTMLElement>('.btn_city_festival')?.classList.contains('disabled'), { onError: () => false })
      return true;
    }
    else if (operation === MunicipalEvent.TheatrePerformances) {
      document.querySelector<HTMLElement>('.btn_theater_plays')?.click();
      await waitWhile(() => !document.querySelector<HTMLElement>('.btn_theater_plays')?.classList.contains('disabled'), { onError: () => false })
      return true;
    } else {
      document.querySelector<HTMLElement>('.btn_victory_procession')?.click();
      await waitWhile(() => !document.querySelector<HTMLElement>('.btn_victory_procession')?.classList.contains('disabled'), { onError: () => false })
      return true;
    }
  }

  private closeAgoraDialog() {
    document.querySelector<HTMLElement>('#place_container')?.parentElement?.parentElement?.parentElement?.querySelector<HTMLButtonElement>('button.ui-dialog-titlebar-close')?.click()
  }
}
