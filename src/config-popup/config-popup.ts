...
export default class ConfigPopup extends EventEmitter {
  ...
  constructor() {
    ...
    this.scheduler = this.config.general.scheduler;
  ...
  private initEventListeners(): void {
    ...
    const schedulerCheckbox = container.querySelector('#scheduler');
    ...
    container.querySelectorAll('#button-panel button').forEach(btn =>
      btn.addEventListener('click', () => {
        if ((btn as HTMLButtonElement).type === 'reset') {
          ...
          (schedulerCheckbox as HTMLInputElement)!.checked = false;
          this.scheduler = false;
        }

        const conflictingCitiesChanged = Object.keys(this.uniquelySelectedFarmingCitiesPerIsle).some(
          isleId =>
            this.uniquelySelectedFarmingCitiesPerIsle[isleId].name !==
            this.config.farmConfig.farmingCities.find(city => city.isleId === isleId)?.name,
        );
        ...
        if (this.configChanged(managersConfigChanges)) {
          ...
          this.config.general.scheduler = this.scheduler;
  ...
  private async createInitialElements() {
    ...
    (container.querySelector('#scheduler') as HTMLInputElement)!.checked = this.scheduler;
    ...
    (container.querySelector('#time-interval-select') as HTMLSelectElement)!.value =
      this.config.farmConfig.farmInterval.toString() ?? FarmTimeInterval.FirstOption.toString();
  ...
  private async createConfictingCitiesSelects(container: HTMLElement) {
    ...
    const arrayOfArraysOfCitiesOnTheSameIsland = uniqueIsleGrids.map(isleId =>
      listOfCities.filter(city => city.isleId === isleId),
    );
      ...
      for (const arrayOfCities of arrayOfArraysOfCitiesOnTheSameIsland) {
        ...
        const initialCity =
          arrayOfCities.find(
            city =>
              city.name ===
              this.config.farmConfig.farmingCities.find(city => city.isleId === arrayOfCities[0].isleId)?.name,
          ) ?? arrayOfCities[0];
...
// export default class ConfigPopup extends EventEmitter {
//   private static instance: ConfigPopup;
//   private configManager!: ConfigManager;
//   private config!: TConfig;
...
//     if (!ConfigPopup.instance) {
//       ConfigPopup.instance = new ConfigPopup();

//       ConfigPopup.instance.configManager = await ConfigManager.getInstance();
//       ConfigPopup.instance.config = ConfigPopup.instance.configManager.getConfig();

//       ConfigPopup.instance.farm = ConfigPopup.instance.config.general.farm;

//       ConfigPopup.instance.builder = ConfigPopup.instance.config.general.builder;
//       ConfigPopup.instance.minimumTracking = ConfigPopup.instance.config.builder.minimumTracking;

//       ConfigPopup.instance.scheduler = ConfigPopup.instance.config.general.scheduler;
//       ConfigPopup.instance.guard = ConfigPopup.instance.config.general.guard;
//       ConfigPopup.instance.recruiter = ConfigPopup.instance.config.general.recruiter;
//       ConfigPopup.instance.masterQueue = ConfigPopup.instance.config.general.masterQueue;
//       ConfigPopup.instance.masterQueueAutoReevaluate = ConfigPopup.instance.config.masterQueue.autoReevaluate;
//       ConfigPopup.instance.farmInterval = ConfigPopup.instance.config.farmConfig.farmInterval;
//       ConfigPopup.instance.humanize = ConfigPopup.instance.config.farmConfig.humanize;

//       return ConfigPopup.instance;
//     }
